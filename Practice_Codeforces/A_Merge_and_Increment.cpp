 ...
 *    Created: 2025-07-20 17:48:54
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Regular Contest 202 (Div. 1)
 *    Problem Name: A - Merge and Increment
 *    Problem URL: https://atcoder.jp/contests/arc202/tasks/arc202_a
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    for (ll x : arr) {
        seg.pb(x);
        while (sz(seg) >= 2 && seg[sz(seg) - 1] == seg[sz(seg) - 2]) {
            ll t = seg.back() + 1;
            seg.pop_back();
            seg.pop_back();
            seg.pb(t);
    ...
    while (alive_count > 1) {
        ...
        ll curKey = max(seg[i], seg[j]);
        if (curKey != key) continue;

        ll cost = llabs(seg[i] - seg[j]);
        total_cost += cost;
        ll newVal = curKey + 1;
