 ...
 *    Created: 2025-08-08 18:28:18
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Codeforces Round 914 (Div. 2)
...
set<pii> knight(int x, int y, int a, int b) {
    set<pii> pos;
    int dx[] = {a, a, -a, -a, b, b, -b, -b};
    int dy[] = {b, -b, b, -b, a, -a, a, -a};

    rep(i, 0, 8) { pos.insert({x - dx[i], y - dy[i]}); }
    return pos;
}
...
void solve() {
    ...
    set<pii> king = knight(xK, yK, a, b);
    set<pii> queen = knight(xQ, yQ, a, b);

    int count = 0;
    for (const auto &pos : king) {
        if (queen.count(pos)) {
            ++count;
        }
    }
