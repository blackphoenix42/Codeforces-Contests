/**
 *    Created: 2025-09-24T20:26:42+05:30
 *    Quote: Edge cases aren’t edges; they’re cliffs.
 *    Group: CodeChef - START205A
 *    Problem Name: Reflection Disorder
 *    Problem URL: https://www.codechef.com/START205A/problems/INVDISORD
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
static inline bool gar(const vi &A) {
    int n = (int)A.size() - 1;
    vi P;
    P.reserve(n);
    rep(i, 1, n + 1) if (A[i] != i) P.pb(i);
    int k = (int)P.size();
    if (k == 0) return true;
    rep(t, 0, k) {
        if (A[P[t]] != P[k - 1 - t]) return false;
    }
    return true;
}
void solve() {
