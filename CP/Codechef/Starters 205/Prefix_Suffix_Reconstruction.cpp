/**
 *    Created: 2025-09-24T20:05:41+05:30
 *    Quote: Half measures are as bad as nothing at all.
 *    Group: CodeChef - START205A
 *    Problem Name: Prefix-Suffix Reconstruction
 *    Problem URL: https://www.codechef.com/START205A/problems/PREFSUFREC
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    rep(v, 1, N + 1) {
        ...
        V.insert(V.end(), F[v], v);
        if ((int)V.size() > 2 * N) break;
    }
    if ((int)V.size() != 2 * N) {
        print(-1);
        return;
    ...
    vi F2(N + 1, 0);
    rep(i, 0, N) {
        if (P2[i] < 1 || P2[i] > N || S2[i] < 1 || S2[i] > N) {
            ok = false;
            break;
        }
        ++F2[P2[i]];
