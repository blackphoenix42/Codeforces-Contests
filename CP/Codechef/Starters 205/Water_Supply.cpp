/**
 *    Created: 2025-09-24T20:23:30+05:30
 *    Quote: Greedy is great—when you can prove it.
 *    Group: CodeChef - START205A
 *    Problem Name: Water Supply
 *    Problem URL: https://www.codechef.com/START205A/problems/WATSUP
 *    Time Limit: 3000 ms
 *    Memory Limit: 256 MB
 **/
...
static inline ll pt(ll x, ll a) { return x - (x / a); }

static inline ll invN(ll t, ll a, ll CAP) {
    if (t <= 0) return 0;
    if (a == 1) return CAP + 1;
    ll q = (t - 1) / (a - 1);
    ll y = t + q;
...
static inline ll pe(ll x, ll A) { return (x * (A - 1) + (A - 1)) / A; }
void solve() {
    int N;
    ll K;
    cin >> N >> K;
    ...
    vector<ll> leak(N + 1, 0);
    rep(i, 2, N + 1) cin >> leak[i];

    vvi g(N + 1);
    rep(v, 2, N + 1) g[par[v]].pb(v);

    vi odr;
    odr.reserve(N);
    odr.pb(1);
    for (size_t i = 0; i < odr.size(); ++i) {
        int u = odr[i];
        for (int v : g[u]) odr.pb(v);
    }

    vi tin(N + 1), tout(N + 1), st;
    st.reserve(N);
    int tmr = 0;
    {
        vpii stk;
    ...
    vi lvs;
    lvs.reserve(N);
    rep(u, 1, N + 1) if (g[u].empty()) lvs.pb(u);

    vl flow(N + 1, 0);
    flow[1] = K;
    for (int i = 1; i < (int)odr.size(); ++i) {
        int v = odr[i];
        int p = par[v];
        flow[v] = pt(flow[p], leak[v]);
    }

    vi idx(lvs.size());
    iota(all(idx), 0);
    sort(all(idx), [&](int a, int b) { return tin[lvs[a]] < tin[lvs[b]]; });

    const int L = (int)lvs.size();
    vi LT(L);
    vl LV(L);
    rep(i, 0, L) {
        LT[i] = tin[lvs[idx[i]]];
    ...
    vl pref(L), suff(L);
    rep(i, 0, L) pref[i] = (i ? min(pref[i - 1], LV[i]) : LV[i]);
    for (int i = L - 1; i >= 0; --i)
        suff[i] = (i + 1 < L ? min(suff[i + 1], LV[i]) : LV[i]);

    auto lr = [&](int u) -> pii {
        int Ls = (int)(lb(all(LT), tin[u]) - LT.begin());
        int Rs = (int)(ub(all(LT), tout[u]) - LT.begin()) - 1;
        if (Ls > Rs) return mp(-1, -1);
        return mp(Ls, Rs);
    };
    ...
    rep(u, 1, N + 1) {
        ...
        if (Lx == -1) {
            ...
            if (Lx > 0) best = min(best, pref[Lx - 1]);
            if (Rx + 1 < L) best = min(best, suff[Rx + 1]);
        }
        oMin[u] = best;
    }

    ll bMin = (ll)4e18;
    for (int lf : lvs) bMin = min(bMin, flow[lf]);

    auto can = [&](ll X) -> bool {
        if (bMin >= X) return true;
        vl need(N + 1, 0);
        for (int i = (int)odr.size() - 1; i >= 0; --i) {
            int u = odr[i];
            if (g[u].empty()) {
                need[u] = X;
            } else {
        ...
        rep(v, 2, N + 1) {
            if (oMin[v] < X) continue;
            int p = par[v];
            if (flow[p] >= need[v]) return true;
        }
        return false;
    };
