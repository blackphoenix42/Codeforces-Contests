 ...
 *    Created: 2025-07-23 20:07:26
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START196A
 *    Problem Name: Unsorted Sorting
 *    Problem URL: https://www.codechef.com/START196A/problems/UNSORTSORT
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
int cl(int n, int m, int x) {
    if (m <= 0) return 0;
    vvi dp, nxt;
    dp.assign(n, vi(m + 1, 0));
    FOR(j, 0, n) {
        if ((x == 1 && j == 0) || (x != 1 && j != 0)) {
            dp[j][1] = 1;
        }
    ...
    FOR(i, 1, n) {
        int rP = n - (i - 1);
        int rN = n - i;
        nxt.assign(rN, vi(m + 1, 0));
        FOR(j, 0, rP) {
            FORN(len, 1, m) {
                int ways = dp[j][len];
                if (!ways) continue;
                FOR(jn, 0, rN) {
                    if (i + 1 < x && jn == 0) continue;
                    if (i + 1 == x && jn != 0) continue;
                    bool inc = (jn >= j);
                    int nlen = inc ? len + 1 : 1;
                    if (nlen > m) continue;
                    nxt[jn][nlen] = mod_add(nxt[jn][nlen], ways);
                }
            }
        }
        dp.swap(nxt);
    ...
    int tot = 0;
    for (auto &row : dp) FORN(len, 1, m) tot = mod_add(tot, row[len]);
    return tot;
}
void solve() {
    int n, k, x;
