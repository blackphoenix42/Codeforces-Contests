 ...
 *    Created: 2025-07-23 20:07:20
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START196A
 *    Problem Name: GCD Reduction
 *    Problem URL: https://www.codechef.com/START196A/problems/GRED
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
int gcdi(int a, int b) { return std::gcd(a, b); }
void solve() {
    int N, X, Y;
    cin >> N >> X >> Y;
    ...
    if (U != -1) pool.erase(find(all(pool), U));

    vector<int> ans;
    ans.reserve(N);
    for (int v : pool) ans.pb(v);
    if (U != -1) ans.pb(U);
    ans.pb(W);
    ans.pb(P);
    ans.pb(Y);
