 ...
 *    Created: 2025-07-23 20:07:24
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START196A
 *    Problem Name: Swap Sum
 *    Problem URL: https://www.codechef.com/START196A/problems/SWAPABK
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
Node mN(const Node &L, const Node &R) {
    Node C;
    for (int s = 0; s < 2; ++s) {
        C.add[s] = L.add[s] + R.add[L.nxt[s]];
        C.nxt[s] = R.nxt[L.nxt[s]];
    }
    return C;
}
struct SegmentTree {
    ...
    SegmentTree(int n, ll K, vector<ll> &A, vector<ll> &B)
        : n(n), K(K), A(&A), B(&B) {
        st.resize(4 * n + 4);
        build(1, 1, n);
    }
    ...
    void ptA(int idx, ll val) {
        (*A)[idx] = val;
        update(1, 1, n, idx);
    }
    void ptB(int idx, ll val) {
        (*B)[idx] = val;
        update(1, 1, n, idx);
    }

    ll queryScore() { return st[1].add[0]; }
...
void solve() {
    ...
    CPUtils::SegmentTree seg(N, K, A, B);
