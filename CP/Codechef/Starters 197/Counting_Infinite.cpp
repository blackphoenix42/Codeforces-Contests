 ...
 *    Created: 2025-07-30 20:21:26
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START197A
 *    Problem Name: Counting (Infinite)
 *    Problem URL: https://www.codechef.com/START197A/problems/CNTINF
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
void precompute_fact1orials(int n) {
...
int main() {
    ...
    rep(i, 0, t) {
        ...
        maxN = max(maxN, tests[i].first);
    }
    vl fact(maxN + 1), invfact(maxN + 1);
    fact[0] = 1;
    FORN(i, 1, maxN) fact[i] = fact[i - 1] * i % MOD;
    invfact[maxN] = mod_exp(fact[maxN], MOD - 2);
    FORD(i, maxN, 1) invfact[i - 1] = invfact[i] * i % MOD;
    ...
    for (auto &tc : tests) {
        ...
        FORN(m, 0, K) {
            ...
            ll Km = (K - m) % MOD;
            FORN(x, 0, m) {
                ll term = fact[N] * invfact[N - x] % MOD * invfact[x] % MOD *
                          fact[m] % MOD * invfact[m - x] % MOD *
                          mod_exp(Km, N - x) % MOD;
                Fm = (Fm + term) % MOD;
            }

            cg = (cg + sign * wc % MOD * Fm) % MOD;
        }
