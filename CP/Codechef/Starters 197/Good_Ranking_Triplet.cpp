 ...
 *    Created: 2025-07-30 20:09:24
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START197A
 *    Problem Name: Good Ranking (Triplet)
 *    Problem URL: https://www.codechef.com/START197A/problems/GOODRANK3
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    if (N <= 4) {
        ...
        FOR(bit, 0, mb) {
            ...
            array<vi, 4> perms;
            perms[0].insert(perms[0].end(), all(z));
            perms[0].insert(perms[0].end(), all(o));
            perms[1].insert(perms[1].end(), all(o));
            perms[1].insert(perms[1].end(), all(z));
            perms[2].insert(perms[2].end(), z.rbegin(), z.rend());
            perms[2].insert(perms[2].end(), o.rbegin(), o.rend());
            perms[3].insert(perms[3].end(), o.rbegin(), o.rend());
            perms[3].insert(perms[3].end(), z.rbegin(), z.rend());

            for (auto &p : perms) res.pb(p);
        }
    }
    print(sz(res));
    for (auto &perm : res) {
