 ...
 *    Created: 2025-07-30 20:21:29
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: CodeChef - START197A
 *    Problem Name: Counting (Finite)
 *    Problem URL: https://www.codechef.com/START197A/problems/CNTFIN
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    vl w(size, 0), sw(size, 0), ab(size), cd(size);

    auto idx = [&](ll x, ll y, ll z) { return (x * S + y) * S + z; };
        ...
        FORN(x, 0, k) {
            FORN(y, 0, x) {
                FORN(z, 0, y) {
                    ll id = idx(x, y, z);
                    ll wy = w[id], s = sw[id];
                    if (wy == 0 && s == 0) continue;

                    auto apply = [&](bool cond, ll nx, ll ny, ll nz, ll mul,
                                     bool aw = false) {
                        if (!cond) return;
                        ll nid = idx(nx, ny, nz);
                        ll inc = aw ? (s + wy) % MOD : s;
                        ab[nid] = (ab[nid] + mod_mul(mul, wy)) % MOD;
                        cd[nid] = (cd[nid] + mod_mul(mul, inc)) % MOD;
                    };

                    apply(x < k, x + 1, y + 1, z + 1, k - x);
                    apply(y > 0, x, y, z, y);
                    apply(y > z, x, y - 1, z, y - z);
                    apply(z > 0, x, y - 1, 0, z, true);
                }
            }
        }
        swap(w, ab);
