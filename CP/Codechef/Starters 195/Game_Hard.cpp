 ...
 *    Created: 2025-07-16 20:15:39
 *    Profile: https://codeforces.com/profile/IndianTourist01
 *    Group: CodeChef - START195A
 *    Problem Name: Game (Hard)
 *    Problem URL: https://www.codechef.com/START195A/problems/GAMEHD
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
struct MaxLineTree {
    struct Line {
        ll slope, intercept;
        ll eval(ll x) const { return slope * x + intercept; }
    };
    struct Node {
        Line line;
    ...
    MaxLineTree(ll lo, ll hi) : xmin(lo), xmax(hi) {}

    void insert(Line new_line) { add(root, xmin, xmax, new_line); }

    ll get_max(ll x) const { return query(root, xmin, xmax, x); }

   private:
    void add(Node *&node, ll l, ll r, Line new_line) {
        if (!node) {
        ...
        if (mid) swap(new_line, node->line);
        if (r == l) return;

        if (left != mid)
            add(node->left, l, m, new_line);
        else
            add(node->right, m + 1, r, new_line);
    }
    ...
    ll query(Node *node, ll l, ll r, ll x) const {
        ...
        ll m = (l + r) / 2;
        if (x <= m)
            return max(val, query(node->left, l, m, x));
        else
            return max(val, query(node->right, m + 1, r, x));
    }
};

int n, maxC;
vector<ll> input, sortedVals, prefix;
vector<vector<pair<ll, ll>>> tree;

void push_line(int idx, int l, int r, int ql, int qr, pair<ll, ll> ln) {
    if (ql > r || qr < l) return;
    if (ql <= l && r <= qr) {
        tree[idx].pb(ln);
        return;
    ...
    int mid = (l + r) / 2;
    push_line(idx * 2, l, mid, ql, qr, ln);
    push_line(idx * 2 + 1, mid + 1, r, ql, qr, ln);
}

void compute(int idx, int l, int r, MaxLineTree &lct, vector<ll> &res) {
    auto saved = lct.root;

    for (auto &[m, b] : tree[idx]) {
        lct.insert({m, b});
    }

    if (l == r) {
        ll c = l;
        ll best = lct.get_max(c);
        res[c] = (best - c * c) / 2;
    } else {
        int mid = (l + r) / 2;
        compute(idx * 2, l, mid, lct, res);
        compute(idx * 2 + 1, mid + 1, r, lct, res);
    }
...
void solve() {
    ...
    sortedVals = input;
    sort(all(sortedVals), greater<>());
    prefix.assign(n + 1, 0);
    for (int i = 1; i <= n; ++i) prefix[i] = prefix[i - 1] + sortedVals[i - 1];

    maxC = 2 * n;
    tree.assign(4 * (maxC + 1), {});

    for (ll p = 1; p <= n; ++p) {
        ll slope = 4 * p - 1;
        ll intercept = 2 * prefix[p] - p * (3 * p - 1);
        int left = p, right = min(2 * p, (ll)maxC);
        push_line(1, 1, maxC, left, right, {slope, intercept});
    }

    vector<ll> dp(maxC + 1, 0);
    MaxLineTree lct(1, maxC);
    compute(1, 1, maxC, lct, dp);

    for (int i = 2; i <= maxC; ++i) dp[i] = max(dp[i], dp[i - 1]);

    for (int i = 1; i <= maxC; ++i) cout << dp[i] << " \n"[i == maxC];
}
