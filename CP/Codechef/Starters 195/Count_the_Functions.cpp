 ...
 *    Created: 2025-07-16 20:16:37
 *    Profile: https://codeforces.com/profile/IndianTourist01
 *    Group: CodeChef - START195A
 *    Problem Name: Count the Functions
 *    Problem URL: https://www.codechef.com/START195A/problems/CTF
 *    Time Limit: 4000 ms
 *    Memory Limit: 256 MB
 **/
...
ll mod_inv(ll a) { return modpow(a, MOD - 2); }
...
void ntt(vector<ll> &a, bool invert) {
    ...
    if ((int)roots.size() < n) {
        ...
        roots.resize(n);
        while ((1 << k) < n) {
            ll w = modpow(PRIM_ROOT, (MOD - 1) >> (k + 1));
            for (int i = 1 << (k - 1); i < (1 << k); i++) {
                roots[2 * i] = roots[i];
                roots[2 * i + 1] = roots[i] * w % MOD;
            }
            k++;
        }
    }
...
vector<ll> multiply(const vector<ll> &A, const vector<ll> &B, int need) {
...
vector<ll> poly_inv(const vector<ll> &A, int n) {
    vector<ll> B(1, modpow(A[0]));
    int sz = 1;
    while (sz < n) {
        sz <<= 1;
        vector<ll> A_cut(min((int)A.size(), sz));
        for (int i = 0; i < (int)A_cut.size(); i++) A_cut[i] = A[i];
        auto tmp = multiply(multiply(B, B, sz), A_cut, sz);
        B.resize(sz);
        for (int i = 0; i < sz; i++)
            B[i] = (2LL * B[i] % MOD - tmp[i] + MOD) % MOD;
    }
    B.resize(n);
    return B;
}

vector<ll> poly_log(const vector<ll> &A, int n) {
    vector<ll> deriv(A.size());
    for (int i = 1; i < (int)A.size(); i++) deriv[i - 1] = A[i] * i % MOD;
    auto invA = poly_inv(A, n);
    auto Q = multiply(deriv, invA, n - 1);
    vector<ll> L(n);
    for (int i = 1; i < n; i++) L[i] = Q[i - 1] * modpow(i) % MOD;
    return L;
}
...
vector<ll> poly_exp(const vector<ll> &A, int n) {
    ...
    while (sz < n) {
        sz <<= 1;
        auto L = poly_log(E, sz);
        vector<ll> diff(sz);
        for (int i = 0; i < sz; i++) {
            ll ai = i < (int)A.size() ? A[i] : 0;
            diff[i] = (ai - L[i] + MOD) % MOD;
        }
        diff[0] = (diff[0] + 1) % MOD;
        E = multiply(E, diff, sz);
    }
    E.resize(n);
    return E;
}

void solve(vector<pair<int, ll>> &tests, int Nmax) {
    vector<ll> G(Nmax + 1);
    G[0] = 1;
    for (int m = 1; m <= Nmax; m++) G[m] = modpow(m + 1, m - 1);
    auto S = poly_log(G, Nmax + 1);
    for (auto &tk : tests) {
        int N = tk.first;
        ll K = tk.second % MOD;
        vector<ll> SK(N + 1);
        for (int i = 0; i <= N; i++) SK[i] = S[i] * K % MOD;
        auto F = poly_exp(SK, N + 1);
        cout << F[N] << "\n";
    }
}
...
int main() {
    ...
    int T = 1;
    cin >> T;
    vector<pair<int, ll>> tests(T);
    int Nmax = 0;
    for (int i = 0; i < T; i++) {
        cin >> tests[i].first >> tests[i].second;
        Nmax = max(Nmax, tests[i].first);
    }
    solve(tests, Nmax);
