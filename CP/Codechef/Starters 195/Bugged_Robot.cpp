 ...
 *    Created: 2025-07-16 20:07:25
 *    Profile: https://codeforces.com/profile/IndianTourist01
 *    Group: CodeChef - START195A
 *    Problem Name: Bugged Robot
 *    Problem URL: https://www.codechef.com/START195A/problems/DIRFOLLOW
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    for (ll d = 1; d * d <= N; ++d) {
        if (N % d) continue;
        for (auto [xc, yc] : vector<pair<ll, ll>>{{d, N / d}, {N / d, d}}) {
            {
                ll low = max(0LL, (ll)max(ceil_log2(xc), ceil_log2(yc)));
                ll high = min(xc, yc) - 1;
                if (low <= high) {
                    ll m = low;
                    ans = min(ans, 2 * m + 1);
            ...
            {
                ll low = max<ll>({1, ceil_log2(yc), ceil_log2(xc) + 1});
                ll high = min(xc, yc - 1);
                if (low <= high) {
                    ll m = low;
                    ans = min(ans, 2 * m);
