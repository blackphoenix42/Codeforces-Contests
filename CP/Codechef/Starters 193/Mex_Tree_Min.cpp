...
void solve() {
    ...
    FORD(i, n - 1, 0) {
        int u = bfs[i];
        EACH(ch, tree[u]) { sub[u] += sub[ch]; }
    }

    vector<pii> order;
    FORN(i, 1, n) {
        if (i != root) order.emplace_back(sub[i], i);
    }
    sort(all(order));
    ...
    label[root] = 0;
    int lbl = 1;
    EACH(ord, order) { label[ord.second] = lbl++; }
    ...
    FORN(u, 1, n) {
        ...
        int mex_val = 0;
        while (mex_val <= n && seen[mex_val]) ++mex_val;
        total += mex_val;
    }
