...
ll ans_for_tree() {
    ...
    comp_size.assign(N + 1, vector<int>(N + 1, 0));
    dfs(1, 0);
    for (int u = 1; u <= N; ++u)
        for (int v : adj[u]) {
            if (parent_[v] == u) {
                comp_size[v][u] = subtree[v];
                comp_size[u][v] = N - subtree[v];
            }
        }

    vector<vector<pair<int, ll>>> wadj(N + 1);
    for (int u = 1; u <= N; ++u)
        for (int v : adj[u]) {
            if (u < v) {
                ll w = (ll)comp_size[u][v] * comp_size[v][u];
                wadj[u].emplace_back(v, w);
                wadj[v].emplace_back(u, w);
            }
        }
    ...
    for (int c = 1; c <= N; ++c) {
        ...
        ll initial = total_pairs - sum_comp;
        ll maxd = 0;
        vector<char> vis(N + 1);
        function<void(int, ll)> dfs2 = [&](int u, ll dist) {
            vis[u] = 1;
            maxd = max(maxd, dist);
            for (auto &pr : wadj[u]) {
                int v = pr.first;
                ll w = pr.second;
                if (!vis[v]) dfs2(v, dist + w);
            }
        };
        dfs2(c, 0);
        best = max(best, initial + maxd);
    }
    return best;
}
void solve() {
