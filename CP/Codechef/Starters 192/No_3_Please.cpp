...
void solve() {
    ...
    // Check if the given array is already valid
    int prefixSum = 0;
    bool isValid = true;
    FORN(i, 1, n) {
        prefixSum = (prefixSum + arr[i]) % 3;
        if (prefixSum == 0) {
            isValid = false;
            break;
        }
    ...
    vi prefix(n + 1);
    FORN(i, 1, n) {
        prefix[i] = (prefix[i - 1] + arr[i]) % 3;
        if (prefix[i] == 1 && !cnt1) cnt1 = i;
        if (prefix[i] == 2 && !cnt2) cnt2 = i;
    }

    vi possibleReverses = {0};
    if (cnt1) possibleReverses.pb(cnt1);
    if (cnt2) possibleReverses.pb(cnt2);

    bool canBeFixed = false;
    EACH(revArr, possibleReverses) {
        vi candidate(n + 1);
        int idx = 1;

        // Reverse the prefix
        FORD(i, revArr, 1) candidate[idx++] = arr[i];
        // Append the rest
        FORN(i, revArr + 1, n) candidate[idx++] = arr[i];

        int runningSum = 0;
        bool bad = false;
        FORN(i, 1, n) {
            runningSum = (runningSum + candidate[i]) % 3;
            if (runningSum == 0) {
                bad = true;
                break;
            }
