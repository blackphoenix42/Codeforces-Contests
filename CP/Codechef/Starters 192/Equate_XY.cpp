...
ll getValue(ll val, ll c) {
    if (val < 0) return -val + c;
    return val + c;
}
void check_multiples(ll x, ll y, ll z, ll c, ll &answer) {
    if (x % y != 0) return;
    ll ratio = x / y;
    FORSQ(factor, 1, ratio) {
        if (ratio % factor == 0) {
            ll cand1 = y * factor;
            CP_MIN(answer, getValue(z - cand1, c));
            ll cand2 = ratio / factor;
            if (factor != cand2) {
                ll t2 = y * cand2;
                CP_MIN(answer, getValue(z - t2, c));
            }
        }
    }
}
...
void solve() {
    ...
    check_multiples(x, y, z, c, answer);
    check_multiples(y, x, z, c, answer);
