 ...
 *    Created: 2025-07-13 20:19:11
 *    Profile: https://codeforces.com/profile/IndianTourist01
 *    Group: CodeChef - START194B
 *    Problem Name: Terminal States
 *    Problem URL: https://www.codechef.com/START194B/problems/TERST
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    auto process_segment = [&](int L, int R) {
        int len = R - L;
        vi B(len);
        for (int i = 0; i < len; i++) B[i] = A[L + i];
        vi comp = B;
        sort(all(comp));
        comp.erase(unique(all(comp)), comp.end());
        CPUtils::FenwickTree fw(sz(comp));
        ll ways = 0;
        for (int i = 0; i < len; i++) {
            int v = int(lower_bound(all(comp), B[i]) - comp.begin()) + 1;
            int s = fw.sum(v - 1);
            int dp = s + 1;
            if (dp >= MOD) dp -= MOD;
            fw.add(v, dp);
    ...
    for (int i = 0; i + 1 < N; i++) {
        if (pref[i] < suff[i + 1]) {
            ...
            CPUtils::FenwickTree fw(comp.size());

            long long ways = 0;
            for (int j = 0; j < len; j++) {
                int v = int(lower_bound(comp.begin(), comp.end(), B[j]) -
                            comp.begin()) +
                        1;
                int s = fw.sum(v - 1);
                int dp = s + 1;
