 ...
 *    Created: 2025-07-13 20:14:08
 *    Profile: https://codeforces.com/profile/IndianTourist01
 *    Group: CodeChef - START194B
 *    Problem Name: Good Arrays (Not Cyclic)
 *    Problem URL: https://www.codechef.com/START194B/problems/CNTGOODARRH
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
int mod_add1(int a, int b) {
    a += b;
    return (a >= MOD ? a - MOD : a);
}

int mod_mul1(ll a, ll b) { return int((a * b) % MOD); }
...
void solve() {
    ...
    int total_ways = 1;
    REP(i, n) {
        int allowed = count(all(mat[i]), '1');
        if (allowed == 0) {
            cout << 0 << nl;
            return;
        }
        total_ways = mod_mul1(total_ways, allowed);
    }

    int half = n / 2, shift = n;
    vi prev_dp(1, 1);

    FOR(step, 1, half + 1) {
        int idxA = half - step;
        int idxB = 2 * half - step;
        ...
        FOR(j, 1, n + 1) {
            if (mat[idxA][j - 1] == '1') A[j] = 1;
            if (mat[idxB][j - 1] == '1') B[j] = 1;
        }

        static int delta_freq[201];
        memset(delta_freq, 0, sizeof delta_freq);

        FOR(x, 1, n + 1)
        if (A[x]) FOR(y, 1, n + 1) if (B[y]) delta_freq[y - x + shift]++;

        vector<pii> transitions;
        FOR(d, -(n - 1), n) {
            int freq = delta_freq[d + shift];
            if (freq > 0) transitions.pb({d, freq});
        }

        int old_sz = sz(prev_dp);
        int new_sz = old_sz + (n - 1);
        vi curr_dp(new_sz + 1);

        REP(d, old_sz) {
            if (!prev_dp[d]) continue;
            EACH(p, transitions) {
                int offset = d + p.fi;
                if (offset < 0 || offset > new_sz) continue;
                curr_dp[offset] =
                    mod_add1(curr_dp[offset], mod_mul(prev_dp[d], p.se));
            }
        }
    ...
    int invalid = prev_dp[0];
    int result = total_ways - invalid;
