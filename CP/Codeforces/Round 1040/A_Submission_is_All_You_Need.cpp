 ...
 *    Created: 2025-07-31 20:05:20
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Codeforces Round 1040 (Div. 2)
 *    Problem Name: A. Submission is All You Need
 *    Problem URL: https://codeforces.com/contest/2130/problem/0
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    rep(i, 0, n) {
        int x;
        cin >> x;
        if (x >= 0 && x < (int)cnt.size()) cnt[x]++;
    }
    int p = min(cnt[0], cnt[1]);
    ll ans = 0;
    ans += 2LL * p;
    cnt[0] -= p;
    cnt[1] -= p;
