...
void solve() {
    ...
    FORN(i, 1, n) cin >> p[i];
    // readvec(p);
    CPUtils::FenwickTree bit1(n);
    // rep(i, 0, n) {
    //     int pi = p[i];
    //     ll Bi = bit.sum(n) - bit.sum(pi);
    //     ll Ai = (n - pi) - Bi;
    //     ans += min(Bi, Ai);
    //     bit.add(pi, 1);
    // }
    ...
    ll inv = 0;
    FORN(i, 1, n) {
        V[i] = bit1.range_sum(p[i] + 1, n);
        inv += V[i];
        bit1.add(p[i], 1);
    }
    CPUtils::FenwickTree bit2(n);
    FORD(i, n, 1) {
        U[i] = bit2.range_sum(p[i] + 1, n);
        bit2.add(p[i], 1);
    }
    ll ans = inv;
    FORN(i, 1, n) {
        if (U[i] < V[i]) ans += (ll)(U[i] - V[i]);
    }
    print(ans);
}
