...
void solve() {
    ...
    auto rbp = [&]() {
        ...
        for (int i = 1; i < maxpt; i++) {
            prefix[i] = prefix[i - 1] + (cov[i - 1] ? 1 : 0);
        }
    };
    ...
    while (true) {
        ...
        rep(i, 0, n) {
            if (used[i]) continue;
            int a = intrvl[i].first;
            int b = intrvl[i].second;
            if (dsu.same(a, b)) continue;
            int total_len = b - a;
            int cov_in_range = prefix[b] - prefix[a];
            int gain = total_len - cov_in_range;
            if (gain > bg) {
                bg = gain;
                bi = i;
            }
        ...
        used[bi] = true;
        ans.pb(bi + 1);
        int a = intrvl[bi].first;
        int b = intrvl[bi].second;
        dsu.unite(a, b);
        for (int x = a; x < b; x++) {
            if (!cov[x]) {
                cov[x] = true;
