 ...
 *    Created: 2025-08-01 20:36:08
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Codeforces Round 1040 (Div. 2)
 *    Problem Name: E1. Interactive RBS (Easy Version)
...
void solve() {
    ...
    if (n >= 2) {
        ...
        for (int attempt = 0; attempt < 25 && idxOpen == -1; ++attempt) {
            int j = dist(rng);
            int f = query4(1, j);
            if (f == 3) {
                idxOpen = 1;
    ...
    if (idxOpen == -1) {
        for (int start = 1; start <= 2 && idxOpen == -1; ++start) {
    ...
    for (int k = 1; k <= n; ++k) {
        if (k == idxOpen || k == idxClose) continue;
        int f = query3(idxOpen, k);
        if (f == 0) {
            answer[k - 1] = '(';
        } else {
            answer[k - 1] = ')';
