...
void solve() {
    ...
    for (int bit = 30; bit >= 0; --bit) {
        ...
        vector<ll> cost0(n), cost1(n);
        for (int i = 0; i < n; i++) {
            ll rem = b[i] & (M2 - 1);
            cost0[i] = (rem < M ? 0 : (M2 - rem));
            cost1[i] = (rem >= M ? 0 : (M - rem));
        }

        vector<array<ll, 2>> dp2(n, {INF, INF});
        vector<array<ll, 2>> parent(n, {-1, -1});
        ...
        for (int i = 1; i < n; i++) {
            ...
            for (int cur = 0; cur < 2; cur++) {
                ll cc = (cur ? cost1[i] : cost0[i]);
                ll best = INF, from = -1;
                for (int prv = 0; prv < 2; prv++) {
                    ll pc = dp2[i - 1][prv];
                    if (pc >= INF) continue;
                    bool ok =
                        want ? (prv == 1 && cur == 1) : !(prv == 1 && cur == 1);
                    if (!ok) continue;
                    ll cand = pc + cc;
                    if (cand < best) {
                        best = cand;
        ...
        ll e0 = dp2[n - 1][0], e1 = dp2[n - 1][1];
        ll bestCost = min(e0, e1);
        if (bestCost >= INF) {
            print(-1);
            return;
        }
