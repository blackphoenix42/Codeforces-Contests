...
ll find_p(ll L, ll R) {
    const int BITS = 61;
    vector<bool> allowed(BITS);
    vl maxSuffix(BITS + 1);
    FOR(i, 0, BITS) allowed[i] = ((L >> i) & 1) == 0;
    maxSuffix[0] = 0;
    FORN(i, 1, BITS) {
        maxSuffix[i] =
            maxSuffix[i - 1] + (allowed[i - 1] ? (1LL << (i - 1)) : 0);
    }
    ...
    for (int pos = BITS - 1; pos >= 0; pos--) {
        ...
        for (int bit = 0; bit <= 1; bit++) {
            if (!allowed[pos] && bit == 1) continue;
            if (tightL && bit < lbit) continue;
            if (tightR && bit > rbit) continue;
            bool nextTightL = tightL && (bit == lbit);
            bool nextTightR = tightR && (bit == rbit);
            ll nextPref = (prefix << 1) | bit;
            ll minVal = nextPref << pos;
            ll maxVal = minVal + maxSuffix[pos];
            if (maxVal < L || minVal > R) continue;
            prefix = nextPref;
            tightL = nextTightL;
            tightR = nextTightR;
            placed = true;
