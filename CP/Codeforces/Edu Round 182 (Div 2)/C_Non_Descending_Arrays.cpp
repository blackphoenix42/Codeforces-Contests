 ...
 *    Created: 2025-09-15T20:11:41+05:30
 ...
 *    Problem URL: https://codeforces.com/contest/2144/problem/C
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    auto A = [&](int i, int s) { return s ? b[i] : a[i]; };
    auto B = [&](int i, int s) { return s ? a[i] : b[i]; };
    ...
    rep(i, 1, n) {
        ...
        if (A(i - 1, 0) <= A(i, 0) && B(i - 1, 0) <= B(i, 0))
            ndp0 = (ndp0 + dp0) % MOD;
        if (A(i - 1, 1) <= A(i, 0) && B(i - 1, 1) <= B(i, 0))
            ndp0 = (ndp0 + dp1) % MOD;
