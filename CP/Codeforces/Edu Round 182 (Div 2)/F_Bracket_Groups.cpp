 ...
 *    Created: 2025-09-15T20:37:50+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: AC first. Optimize second.
 *    Group: Codeforces - Educational Codeforces Round 182 (Rated for Div. 2)
 *    Problem Name: F. Bracket Groups
 *    Problem URL: https://codeforces.com/contest/2144/problem/F
 *    Time Limit: 2000 ms
 *    Memory Limit: 512 MB
 **/
...
static inline int enc(int pos, int bal, int st, int B, int S) {
    return (pos * (B + 1) + bal) * S + st;
}

static string build_word(const vs &forbid, int k) {
    if (forbid.empty()) {
        string t;
        t.reserve(k);
        for (int i = 0; i < k / 2; i++) {
    ...
    CPUtils::AC ac;
    for (auto &x : forbid) ac.add(x);
    ac.build();
    int S = (int)ac.go.size();
    int B = k / 2;
    int TOT = (k + 1) * (B + 1) * S;
    vc vis(TOT, 0);
    vi parent(TOT, -1);
    ...
    while (!q.empty()) {
        ...
        for (int c = 1; c >= 0; c--) {
            ...
            int ns = ac.go[st][c];
            if (ac.term[ns]) continue;
            int nid = enc(pos + 1, nb, ns, B, S);
            if (!vis[nid]) {
                vis[nid] = 1;
                parent[nid] = id;
                step[nid] = (c ? '(' : ')');
...
static bool feasible(const vector<string> &forbid, int k) {
    string s = build_word(forbid, k);
    return !s.empty();
}
...
void solve() {
    ...
    rep(i, 1, n + 1) {
        ...
        for (auto &g : groups) {
            vs forbid;
            forbid.reserve(g.size() + 1);
            for (int idx : g) forbid.pb(a[idx]);
            forbid.pb(a[i]);
            if (feasible(forbid, k)) {
                g.pb(i);
                placed = true;
                break;
            }
        }
        if (!placed) groups.pb(vi{i});
    }
    print(groups.size());
    for (auto &g : groups) {
        vs forbid;
        forbid.reserve(g.size());
        for (int idx : g) forbid.pb(a[idx]);
        string r = build_word(forbid, k);
        print(r);
        print(g.size());
