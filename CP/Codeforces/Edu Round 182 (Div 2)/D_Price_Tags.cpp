 ...
 *    Created: 2025-09-15T20:18:15+05:30
 ...
 *    Problem Name: D. Price Tags
 *    Problem URL: https://codeforces.com/contest/2144/problem/D
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
void solve() {
    ...
    vi pref(maxC + 1, 0);
    rep(i, 1, maxC + 1) pref[i] = pref[i - 1] + freq1[i];
    ...
    for (int x = 2; x <= maxC + 1; ++x) {
        ...
        rep(v, 1, vmax + 1) {
            ...
            if (!cnt) continue;
            sumNew += 1LL * v * cnt;
            if (freq1[v]) match += min<ll>(cnt, freq1[v]);
        }

        ll income = sumNew - y * (ll)(n - match);
