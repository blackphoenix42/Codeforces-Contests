...
ll solve(ll n) {
    if (n <= 0) return 0;
    static const ll MOD = 210;
    static const ll PHI = 48;

    static const array<int, 211> pref = []() {
        array<int, 211> p{};
        for (int i = 1; i <= 210; ++i) {
            bool ok = (i % 2) && (i % 3) && (i % 5) && (i % 7);
            p[i] = p[i - 1] + ok;
        }
        return p;
    }();
