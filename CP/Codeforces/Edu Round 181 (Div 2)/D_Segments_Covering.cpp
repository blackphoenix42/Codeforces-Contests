...
ll mod_inv(ll a) { return mod_pow((a % MOD + MOD) % MOD, MOD - 2); }
...
void solve() {
    ...
    vector<Seg> segs(n);
    for (auto &s : segs) cin >> s.l >> s.r >> s.p >> s.q;

    vector<vector<pair<int, ll>>> start_at(m + 2);
    ll P0 = 1;

    for (const auto &s : segs) {
        ll num = (s.q - s.p) % MOD;
        if (num < 0) num += MOD;
        P0 = (P0 * num % MOD) * mod_inv(s.q) % MOD;

        ll w = (ll)s.p % MOD * mod_inv((s.q - s.p) % MOD + MOD) % MOD;
        start_at[s.l].push_back({s.r, w});
    }
    ...
    FORD(pos, m, 1) {
        ll cur = 0;
        for (auto &[r, w] : start_at[pos]) {
            cur = (cur + w * tot[r + 1]) % MOD;
        }
        tot[pos] = cur;
    }
