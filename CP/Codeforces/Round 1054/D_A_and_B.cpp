 ...
 *    Created: 2025-09-25T20:12:41+05:30
 ...
 *    Problem URL: https://codeforces.com/contest/2149/problem/D
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
static inline ll ctb(const string &s, char ch) {
    vi pos;
    ...
    if (k <= 1) return 0;
    vl shift(k);
    rep(i, 0, k) shift[i] = (ll)pos[i] - i;
    nth_element(shift.begin(), shift.begin() + k / 2, shift.end());
    ll med = shift[k / 2];
    ll ans = 0;
    for (ll v : shift) ans += llabs(v - med);
    return ans;
}
