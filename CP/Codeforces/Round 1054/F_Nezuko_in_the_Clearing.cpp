 ...
 *    Created: 2025-09-25T20:12:52+05:30
 *    Profile: https://codeforces.com/profile/PhoenixIzHere
 *    Quote: Think DP; code iteratively; test mercilessly.
 *    Group: Codeforces - Codeforces Round 1054 (Div. 3)
 *    Problem Name: F. Nezuko in the Clearing
 *    Problem URL: https://codeforces.com/contest/2149/problem/F
 *    Time Limit: 1000 ms
 *    Memory Limit: 256 MB
 **/
...
static inline i128 tri(ll m) { return (i128)m * (m + 1) / 2; }

static inline ll mR(ll H) {
    if (H <= 1) return 0;
    ld x = sqrtl(1.0L + 8.0L * (H - 1)) - 1.0L;
    ll m = (ll)floor(x / 2.0L);
    while (m > 0 && tri(m) >= H) --m;
    while (tri(m + 1) < H) ++m;
    return m;
}
...
void solve() {
    ...
    while (lo <= hi) {
        ...
        i128 Smin = (i128)r * tri(q + 1) + (i128)(b - r) * tri(q);
        i128 rhs = (i128)h + R - 1;
