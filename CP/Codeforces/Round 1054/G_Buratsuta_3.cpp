 ...
 *    Created: 2025-09-25T20:12:57+05:30
 *    Profile: https://codeforces.com/profile/PhoenixIzHere
 *    Quote: Debugging: where assumptions go to die.
 *    Group: Codeforces - Codeforces Round 1054 (Div. 3)
 *    Problem Name: G. Buratsuta 3
 *    Problem URL: https://codeforces.com/contest/2149/problem/G
 *    Time Limit: 4500 ms
 *    Memory Limit: 256 MB
 **/
...
struct SegmentTree {
    ...
    SegmentTree(const vl &arr) : n((int)arr.size()), a(arr) {
        st.resize(4 * n);
        build(1, 0, n - 1);
    }
    static void push(Node &nd, ll val, int add) {
        while (add > 0) {
            if (nd.k1 > 0 && nd.c1 == val) {
                nd.k1 += add;
                return;
            }
            if (nd.k2 > 0 && nd.c2 == val) {
                nd.k2 += add;
                return;
            }
            if (nd.k1 == 0) {
    ...
    static Node merge(const Node &L, const Node &R) {
        Node res = L;
        if (R.k1) push(res, R.c1, R.k1);
        if (R.k2) push(res, R.c2, R.k2);
        return res;
    }
    ...
    Node query(int l, int r) const { return query(1, 0, n - 1, l, r); }
    ~SegmentTree() = default;
};
struct FenwickTree {
    int n;
...
void solve() {
    ...
    CPUtils::SegmentTree st(a);

    auto CI = [&](ll v, int L, int R) -> int {
        auto it = pos.find(v);
        if (it == pos.end()) return 0;
        const auto &vec = it->second;
        return int(ub(all(vec), R) - lb(all(vec), L));
    };
    ...
    while (q--) {
        ...
        CPUtils::Node cand = st.query(l - 1, r - 1);
        vl ans;
        vl cands;
        if (cand.k1 > 0 && cand.c1 != -1) cands.pb(cand.c1);
        if (cand.k2 > 0 && cand.c2 != -1 && cand.c2 != cand.c1)
            cands.pb(cand.c2);

        for (ll v : cands) {
            int freq1 = CI(v, l, r);
            if (freq1 > thr) ans.pb(v);
        }
        if (ans.empty()) {
            print(-1);
        } else {
