 ...
 *    Created: 2025-09-25T20:18:55+05:30
 *    Profile: https://codeforces.com/profile/PhoenixIzHere
 *    Quote: Binary search is a state of mind.
 *    Group: Codeforces - Codeforces Round 1054 (Div. 3)
 *    Problem Name: E. Hidden Knowledge of the Ancients
 *    Problem URL: https://codeforces.com/contest/2149/problem/E
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
static inline ll amdwml(const vl &a, int K, int X) {
    if (K <= 0 || X <= 0) return 0;
    umap1<ll, int> cnt;
...
void solve() {
    ...
    vl a(n);
    readvec(a);
    auto f = [&](int K, int X) -> ll { return amdwml(a, K, X); };
    ll ans = (f(k, r) - f(k, l - 1)) - (f(k - 1, r) - f(k - 1, l - 1));
    print(ans);
}
