...
int n, k;
ll w[MAXN + 5];
int c0[MAXN + 5], c2[MAXN + 5];
vi adj[MAXN + 5];
int parent_[MAXN + 5], depth_[MAXN + 5];
int up[LOG][MAXN + 5];
int tin[MAXN + 5];
int timer;
...
void solve() {
    ...
    byColor.reserve(k + 1);
    for (int i = 1; i <= n; i++) {
        byColor[c2[i]].push_back(i);
    }

    vc isCutie(n + 1, 0);
    ll answer = 0;
    for (int color = 1; color <= k; color++) {
        auto &vec = byColor[color];
