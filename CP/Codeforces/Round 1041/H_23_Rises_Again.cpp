 ...
 *    Created: 2025-08-07 20:14:29
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Atto Round 1 (Codeforces Round 1041, Div. 1 + Div. 2)
 *    Problem Name: H. 23 Rises Again
 *    Problem URL: https://codeforces.com/contest/2127/problem/H
 *    Time Limit: 5000 ms
...
int dfs_dp(int v, int used) {
    int &ans = dp_cache[v][used];
    if (dp_seen[v][used]) return ans;
    dp_seen[v][used] = true;

    int cap = bcap[v] - used;
    if (cap < 0) return ans = -INF;
    int base = 0;
    static int gains[30];
    int gsz = 0;
    // vi diffs;
    // diffs.reserve(children[v].size());
    for (int w : children[v]) {
        int out_edge = dfs_dp(w, 0);
        int with_edge = dfs_dp(w, 1);
        base += out_edge;
        gains[gsz++] = (with_edge + 1) - out_edge;
    }
    sort(gains, gains + gsz, greater<int>());
    for (int i = 0; i < cap && i < gsz && gains[i] > 0; ++i) {
        base += gains[i];
    }
    return ans = base;
}
int tree_dp() {
    memset(dp_seen, 0, sizeof(dp_seen));
    return dfs_dp(0, 0);
}
void solve() {
    cin >> n >> m;
    ...
    int c = sz(extraEdges);
    for (int v = 0; v < n; v++) bcap[v] = 2;
    int best = tree_dp();
    auto try_with = [&](const vector<int> &pick) {
        for (int v = 0; v < n; v++) bcap[v] = 2;
        for (int ei : pick) {
            auto [u, v] = edges[ei];
            if (--bcap[u] < 0 || --bcap[v] < 0) return;
        }
        int tdp = tree_dp();
        best = max(best, (int)pick.size() + tdp);
    };

    // inc_count.assign(n, 0);
    // bcap.assign(n, 2);
    // int bestTreeOnly = dfs_dp(0, 0);
    // int bestOverall = bestTreeOnly;

    // int full = 1 << c;
    // rep(mask, 1, full) {
    //     int bcnt = __builtin_popcount(mask);
    //     if (bcnt + bestTreeOnly <= bestOverall) continue;
    //     bool ok = true;
    //     for (int v = 0; v < n; v++) inc_count[v] = 0;
    //     for (int i = 0; i < c; i++) {
    //         if (mask & (1 << i)) {
    //             auto [u, v] = edges[extraEdges[i]];
    //             if (++inc_count[u] > 2 || ++inc_count[v] > 2) {
    //                 ok = false;
    //                 break;
    //             }
    //         }
    //     }
    //     if (!ok) continue;
    //     rep(v, 0, n) { bcap[v] = 2 - inc_count[v]; }
    //     int takeTree = dfs_dp(0, 0);
    //     int total = bcnt + takeTree;
    //     if (total > bestOverall) {
    //         bestOverall = total;
    //     }
    // }
    for (int k = 1; k <= 5 && best < n; k++) {
        vector<int> pick(k);
        function<void(int, int)> gen = [&](int idx, int start) {
            ...
            for (int j = start; j + (k - idx) <= c; j++) {
                pick[idx] = extraEdges[j];
                gen(idx + 1, j + 1);
                if (best == n) return;
            }
        };
