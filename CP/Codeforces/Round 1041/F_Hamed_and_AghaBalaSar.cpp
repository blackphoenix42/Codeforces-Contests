 ...
 *    Problem Name: F. Hamed and AghaBalaSar
 *    Problem URL: https://codeforces.com/contest/2127/problem/F
 *    Time Limit: 2000 ms
 *    Memory Limit: 512 MB
 **/
...
int main() {
    ...
    int MAXF = MAXN + MAXM + 5;

    vl fac(MAXF), ifac(MAXF);
    fac[0] = 1;
    rep(i, 1, MAXF) { fac[i] = fac[i - 1] * i % MOD; }
    ifac[MAXF - 1] = mod_pow(fac[MAXF - 1]);
    FORD(i, MAXF - 1, 1) { ifac[i - 1] = ifac[i] * i % MOD; }

    vi inv(MAXN + 1);
    inv[1] = 1;
    FORN(i, 2, MAXN) {
        inv[i] = (int)(MOD - (ll)(MOD / i) * inv[MOD % i] % MOD);
    }
    ...
    auto g = [&](int k, int s) -> ll {
        if (s < 0) return 0;
        if (k == 0) return s == 0;
        return C(s + k - 1, k - 1);
    };
    auto W = [&](int k, int s, int H) -> ll {
        ll res = 0;
        rep(t, 0, k + 1) {
            int rem = s - t * (H + 1);
            if (rem < 0) break;
            ll ways = g(k, rem);
            ll term = C(k, t) * ways % MOD;
            if (t & 1) {
                res = (res + MOD - term) % MOD;
            } else {
                res = (res + term) % MOD;
            }
        }
    ...
    while (T--) {
        ...
        rep(H, 0, m + 1) {
            ...
            ll num1 = ((ll)H * A % MOD - S) % MOD;
            if (num1 < 0) num1 += MOD;
            ll t1 = num1 * inv[A] % MOD;
            ans = (ans + W1 * t1) % MOD;

            if (n >= 3 && S >= H) {
                ll W2 = W(A - 1, S - H, H);
                ll num2 = ((ll)H * (A - 1) % MOD - (S - H)) % MOD;
                if (num2 < 0) num2 += MOD;
                ll t2 = num2 * inv[A - 1] % MOD;
                ans = (ans + (ll)(n - 2) * W2 % MOD * t2) % MOD;
            }
        }
        print(ans);
    }
