 ...
 *    Problem Name: D. Root was Built by Love, Broken by Destiny
 *    Problem URL: https://codeforces.com/contest/2127/problem/D
 *    Time Limit: 2000 ms
 *    Memory Limit: 256 MB
 **/
...
#ifndef ONLINE_JUDGE
ll fact[MAXN1], inv_fact[MAXN1];
#else
static ll fact[MAXN1], inv_fact[MAXN1];
#endif
...
void solve(vvi &graph, vi &color, vi &vis, vi &vis_cycle, vl &fac) {
    int n, m;
    cin >> n >> m;

    rep(i, 1, n + 1) {
        graph[i].clear();
        color[i] = vis[i] = vis_cycle[i] = 0;
    }
    ...
    function<bool(int, int)> bipart = [&](int u, int c) {
        color[u] = c;
        for (int v : graph[u]) {
            if (color[v] == 0) {
                if (!bipart(v, 3 - c)) return false;
            } else if (color[v] == c) {
                return false;
            }
        }
    ...
    ll ans = 1;
    bool isCycle = false, is_valid = true, isCorner = false;
    int comp = 0;

    function<void(int, int)> dfs_cycle = [&](int u, int parent) {
        vis_cycle[u] = 1;
        for (int v : graph[u]) {
            if (!vis_cycle[v])
                dfs_cycle(v, u);
            else if (v != parent && vis_cycle[v] == 1)
                isCycle = true;
        }
    };
    ...
    function<void(int)> dfs_comp = [&](int u) {
        ...
        if (deg_many > 2) is_valid = false;
        if (deg_many && deg_one + deg_many >= 2) isCorner = true;
        ans = (ans * fac[deg_one]) % MOD;
    };
    ...
    if (isCorner) ans = (ans * 2) % MOD;
    ans = (ans * 2) % MOD;
    ans = (ans * fac[comp]) % MOD;
    print(ans);
}
...
int main() {
    ...
    vvi graph(MAXN);
    vi color(MAXN), vis(MAXN), vis_cycle(MAXN);
    vl fac(MAXN);
    ...
    cin >> t;
    while (t--) {
        solve(graph, color, vis, vis_cycle, fac);
    }
