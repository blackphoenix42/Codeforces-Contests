 ...
 *    Created: 2025-07-27 20:08:25
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Codeforces Round 1039 (Div. 2)
 *    Problem Name: B. Deque Process
 *    Problem URL: https://codeforces.com/contest/2128/problem/B
 *    Time Limit: 1500 ms
 *    Memory Limit: 256 MB
 **/
...
bool valid(int val, int l, int inc, int dec) {
    int ninc = (val > l ? inc + 1 : 1);
    int ndec = (val < l ? dec + 1 : 1);
    return (ninc < 5 && ndec < 5);
}
bool dead(deque<int> &p, int val, int inc, int dec, bool l) {
    int sz = p.size();
    ...
    int inc1 = (next1 > val ? inc + 1 : 1);
    int dec1 = (next1 < val ? dec + 1 : 1);
    int inc2 = (next2 > val ? inc + 1 : 1);
    int dec2 = (next2 < val ? dec + 1 : 1);

    bool safe1 = (inc1 < 5 && dec1 < 5);
    bool safe2 = (inc2 < 5 && dec2 < 5);
...
void solve() {
    ...
    auto ct = [&](int l1, int inc1, int dec1) {
        int left = p.front(), right = p.back();

        bool cl = valid(left, l1, inc1, dec1);
        bool cr = valid(right, l1, inc1, dec1);

        if (cl && !cr) return 'L';
        if (!cl && cr) return 'R';
        if (!cl && !cr) return 'L';

        bool dl = dead(p, left, (left > l1 ? inc1 + 1 : 1),
                       (left < l1 ? dec1 + 1 : 1), true);
        bool dr = dead(p, right, (right > l1 ? inc1 + 1 : 1),
                       (right < l1 ? dec1 + 1 : 1), false);
        ...
        // int m = (int)q.size();
        // if (m < 4) return true;
        // bool inc = (q[m - 4] < q[m - 3] && q[m - 3] < q[m - 2] &&
        //             q[m - 2] < q[m - 1] && q[m - 1] < val);
        // bool dec = (q[m - 4] > q[m - 3] && q[m - 3] > q[m - 2] &&
        //             q[m - 2] > q[m - 1] && q[m - 1] > val);
        // return !(inc || dec);
    };
