 ...
 *    Created: 2025-07-27 21:26:19
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: Codeforces - Codeforces Round 1039 (Div. 2)
 *    Problem Name: E2. Submedians (Hard Version)
 *    Problem URL: https://codeforces.com/contest/2128/problem/E2
 *    Time Limit: 3000 ms
 *    Memory Limit: 512 MB
 **/
...
void solve() {
    ...
    auto process = [&](int m) {
        ...
        if (m == 1) {
            rep(i, 0, wcount) { intrvl.pb({a[i], a[i], i + 1, i + 1}); }
            return;
        }
        ...
        auto getL = [&]() { return *prev(low.end()); };
        auto getR = [&]() {
            return (posR > posL ? *high.begin() : *prev(low.end()));
        };
        intrvl.pb({getL(), getR(), 1, m});
    ...
    sort(all(intrvl), [&](auto &A, auto &B) { return A.L < B.L; });

    vector<pii> answer(n + 1, {-1, -1});
    priority_queue<pair<int, pii>, vector<pair<int, pii>>, greater<>> pq;
    int idx = 0, M = (int)intrvl.size();
    FORN(v, 1, n) {
        while (idx < M && intrvl[idx].L <= v) {
            pq.push({intrvl[idx].R, {intrvl[idx].l, intrvl[idx].r}});
            idx++;
        }
        while (!pq.empty() && pq.top().first < v) {
            pq.pop();
        }
        if (!pq.empty() && answer[v].first == -1) {
            answer[v] = pq.top().second;
        }
    }
    ...
    FORN(v, 1, n) {
        if (answer[v].first != -1) {
            out.emplace_back(v, answer[v].first, answer[v].second);
        }
    }
    print(sz(out));
    for (auto &t : out) {
