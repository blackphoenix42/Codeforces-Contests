 ...
 *    Problem Name: F. Strict Triangle
 *    Problem URL: https://codeforces.com/contest/2128/problem/F
 *    Time Limit: 4000 ms
 *    Memory Limit: 512 MB
 **/
...
void solve() {
    ...
    auto dijkstra = [&](int src, vl &dist, vi &pEdge) {
        dist.assign(n + 1, INF);
        pEdge.assign(n + 1, -1);
        dist[src] = 0;
    ...
    rep(i, 0, m) {
        ...
        if (u == k || v == k) continue;
        ll cost = inQ[i] ? edges[i].r : edges[i].l;
        adj2[u].emplace_back(v, cost);
        adj2[v].emplace_back(u, cost);
    }
