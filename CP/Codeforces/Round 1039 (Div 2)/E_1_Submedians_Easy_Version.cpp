 ...
 *    Problem Name: E1. Submedians (Easy Version)
...
void solve() {
    ...
    vi a(n), b(n);
    readvec(a);
    auto check = [&](int x, bool rec, int &L, int &R) -> pii {
        vi prefix(n + 1, 0);
        FORN(i, 1, n) prefix[i] = prefix[i - 1] + (a[i - 1] >= x ? 1 : -1);
        int mn = 0, idx = 0;
        for (int r = k; r <= n; ++r) {
            if (prefix[r - k] < mn) {
                mn = prefix[r - k];
            ...
            if (prefix[r] - mn >= 0) return make_pair(idx + 1, r);
        }
        return make_pair(-1, -1);

        rep(i, 0, n) b[i] = a[i] >= x ? 1 : -1;
        if (k % 2 == 0) {
            int s = 0;
            rep(i, 0, k) s += b[i];
            if (s >= 0) {
    ...
    while (lo < hi) {
        int mid = (lo + hi + 1) / 2;
        int d1, d2;
        if (check(mid, false, d1, d2).first != -1) {
            ans = mid;
            lo = mid;
        } else
            hi = mid - 1;
    }
    auto k1 = check(lo, true, ans, hi);
    int L = k1.first, R = k1.second;
    cout << lo << " " << L << " " << R << "\n";
}
