 ...
 *    Created: 2025-09-17T20:34:19+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: Proof first, code second, AC third.
 *    Group: Codeforces - Codeforces Round 1051 (Div. 2)
 *    Problem Name: D2. Inversion Graph Coloring (Hard Version)
 *    Problem URL: https://codeforces.com/contest/2143/problem/D2
 *    Time Limit: 3000 ms
 *    Memory Limit: 512 MB
 **/
...
struct FenwickTree {
    ...
    vi f;
    FenwickTree() {}
    FenwickTree(int n_) : n(n_), f(n_ + 1, 0) {}
...
void solve() {
    ...
    int N = n;
    vector<CPUtils::FenwickTree> row(N + 1, CPUtils::FenwickTree(N + 1));
    vector<CPUtils::FenwickTree> col(N + 1, CPUtils::FenwickTree(N + 1));

    auto addRow = [&](int x, int y, int v) { row[x].add(y, v); };
    auto addCol = [&](int y, int x, int v) { col[y].add(x, v); };
    auto sumRow = [&](int x, int upto) {
        return (row[x].sum(upto) - row[x].sum(x - 1) + MOD) % MOD;
    };
    auto sumCol = [&](int y, int upto) { return col[y].sum(min(upto, y)); };
    ...
    for (int v : a) {
        vi s1(v + 1, 0);
        rep(x, 0, v + 1) s1[x] = sumRow(x, v);

        vi s2;
        s2.reserve(N - v);
        rep(y, v + 1, N + 1) s2.pb(sumCol(y, v));
