 ...
 *    Created: 2025-09-17T20:07:21+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: Premature optimization is TLE in disguise.
 *    Group: Codeforces - Codeforces Round 1051 (Div. 2)
 *    Problem Name: D1. Inversion Graph Coloring (Easy Version)
...
void solve() {
    ...
    for (int x : a) {
        ...
        rep(r, 0, n + 1) {
            rep(b, 0, r + 1) {
                int cur = dp[r][b];
                if (!cur) continue;
                ndp[r][b] = mod_add(ndp[r][b], cur);
                if (x >= r) {
                    ndp[x][b] = mod_add(ndp[x][b], cur);
                } else if (x >= b) {
                    ndp[r][x] = mod_add(ndp[r][x], cur);
                }
            }
        }
        dp.swap(ndp);
    ...
    int ans = 0;
    rep(r, 0, n + 1) { rep(b, 0, r + 1) ans = mod_add(ans, dp[r][b]); }
    print(ans);
}
