 ...
 *    Quote: Two pointers: because n^2 hurts.
 *    Group: Codeforces - Codeforces Round 1051 (Div. 2)
 *    Problem Name: E. Make Good
...
static string rbs(int n, int target) {
    int k = n / 2;
    string s;
    s.reserve(n);
    int bal = 0, opens = 0, evens = 0;
    ...
    rep(i, 1, n + 1) {
        ...
        int leftO = k - opens;
        int close = (i - 1) - opens;
        int closeLeft = k - close;
        int evenNeed = target - evens;

        auto allow = [&]() {
            if (leftO <= 0) return false;
            return closeLeft >= (bal + 1);
        };
    ...
    if (bal != 0 || opens != n / 2 || evens != target) return string();
    return s;
}
