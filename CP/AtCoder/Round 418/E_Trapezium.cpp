...
static inline ll pack_u32(uint32_t a, uint32_t b) {
    return (ll(a) << 32) | ll(b);
}
void solve() {
    int N;
    ...
    rep(i, 0, N) {
        rep(j, i + 1, N) {
            ...
            ll slopeKey = pack_u32(uint32_t(dx), uint32_t(dy));
            slopes.pb(slopeKey);

            uint32_t sx = uint32_t(X[i] + X[j]);
            uint32_t sy = uint32_t(Y[i] + Y[j]);
            ll midKey = pack_u32(sx, sy);
            mids.pb(midKey);
        }
    }
