 ...
 *    Created: 2025-08-09 18:55:18
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Beginner Contest 418
 *    Problem Name: G - Binary Operation
 *    Problem URL: https://atcoder.jp/contests/abc418/tasks/abc418_g
 *    Time Limit: 2000 ms
...
static bool can21(const string &s, const Op &op, umap1<string, bool> &memo) {
    auto it = memo.find(s);
    ...
    while (!q.empty()) {
        ...
        for (size_t i = 0; i + 1 < t.size(); ++i) {
            ...
            string u;
            u.reserve(t.size() - 1);
            if (i > 0) u.append(t.begin(), t.begin() + i);
            u.pb(char('0' + x));
            if (i + 2 < t.size()) u.append(t.begin() + i + 2, t.end());
...
static DFA dfaOp(const Op &op) {
    ...
    rep(len, 0, K + 1) {
        for (int m = 0; m < (1 << len); ++m) {
            string t;
            t.resize(len);
            rep(i, 0, len) t[i] = char('0' + ((m >> (len - 1 - i)) & 1));
            suffs.pb(t);
        }
    }

    umap1<string, bool> memo;
    memo.reserve(1 << 14);

    auto feature = [&](const string &s) {
        string v;
        v.resize(suffs.size());
        for (size_t i = 0; i < suffs.size(); ++i) {
            string u = s + suffs[i];
            v[i] = can21(u, op, memo) ? '1' : '0';
        }
        return v;
    };

    umap1<string, int> klass;
    vs rep;
    qi q;
    ...
    trans.resize(1);
    acc.resize(1);
    acc[0] = can21(rep[0], op, memo) ? 1 : 0;

    auto state = [&](const string &s) {
        string fv = feature(s);
        auto it = klass.find(fv);
        if (it != klass.end()) return it->second;
        int id = (int)rep.size();
        klass.emplace(fv, id);
        rep.pb(s);
        trans.pb({{0, 0}});
        acc.pb(can21(s, op, memo) ? 1 : 0);
        q.push(id);
        return id;
    };
...
static pair<int64_t, ll> dfaCnt(const DFA &D, const string &T) {
    const int Q = D.Q;
    vl dp(Q, 0), ndp(Q, 0);
    vi best(Q, INT_MIN), nbest(Q, INT_MIN);
    ...
    for (char ch : T) {
        int b = ch - '0';
        fill(all(ndp), 0);
        fill(all(nbest), INT_MIN);

        rep(q, 0, Q) {
            if (dp[q] == 0) continue;
            int to = D.tr[q][b];
            ndp[to] += dp[q];
            if (best[q] != INT_MIN) nbest[to] = max(nbest[to], best[q] + 1);
        }

        int q1 = D.tr[D.init][b];
        ndp[q1] += 1;
        nbest[q1] = max(nbest[q1], 1);
        ...
        rep(q, 0, Q) {
            if (D.accept[q]) {
                add += dp[q];
                localBest = max(localBest, best[q]);
            }
        }
        M += add;
        bestL = max(bestL, localBest);
    }

    if (bestL == INT_MIN) bestL = -1;
    return {bestL, M};
}
void solve() {
    int N;
    ...
    array<DFA, 16> DFAs;
    rep(idx, 0, 16) {
        Op op{(idx >> 3) & 1, (idx >> 2) & 1, (idx >> 1) & 1, idx & 1};
        DFAs[idx] = dfaOp(op);
    }

    rep(idx, 0, 16) {
        auto [L, M] = dfaCnt(DFAs[idx], T);
        cout << L << ' ' << M << "\n";
    }
}
