 ...
 *    Created: 2025-08-09 17:37:14
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Beginner Contest 418
 *    Problem Name: F - We're teapots
 *    Problem URL: https://atcoder.jp/contests/abc418/tasks/abc418_f
 *    Time Limit: 3000 ms
...
struct SegTree {
    ...
    void set_point(int p, const Mat &v) {
        int i = sz + p;
        t[i] = v;
        for (i >>= 1; i; i >>= 1) t[i] = mul(t[i << 1], t[i << 1 | 1]);
    }
    Mat all_prod() const { return t[1]; }
};
...
void solve() {
    ...
    ifact[MAX - 1] = mod_pow(fact[MAX - 1], MOD - 2);
    FORD(i, MAX - 2, 0) ifact[i] = ifact[i + 1] * (i + 1) % MOD;
    ...
    auto getF = [&](int idx) -> ll {
        if (idx < 0) return 0;
        return F[idx];
    };

    auto A_mat = [&](int L, int k) -> Mat {
        Mat M;
        M.a00 = C(L - k, k);
        M.a01 = C(L - k, k - 1);
        M.a10 = C(L - 1 - k, k);
    ...
    auto updChange = [&]() {
        Mat P = seg.all_prod();
        Mat Tail = B_mat(N - lastIdx);
        Mat PT = mul(P, Tail);
        ll ans = (PT.a00 + PT.a01) % MOD;
        if (ans < 0) ans += MOD;
        print(ans);
    };

    auto prev_it = [&](auto it) {
        if (it == mp.begin()) return it;
        return prev(it);
    };
    ...
    for (int qii = 0; qii < Q; ++qii) {
        ...
        if (y == -1) {
            ...
            auto itR = next(it);
            if (itR != mp.end()) {
                int L = itR->first - itL->first;
                int k = itR->second - itL->second;
                seg.set_point(itR->first, A_mat(L, k));
            }
            seg.set_point(it->first, Mat());
            bool wasLast = (x == lastIdx);
            mp.erase(it);
            if (wasLast) lastIdx = prev(mp.end())->first;
            updChange();
        } else {
            if (it == mp.end()) {
                auto itR = mp.lb(x);
                auto itL = (itR == mp.begin()) ? itR : prev(itR);
                int aL = itL->second;
                if (itR != mp.end()) {
                    int LnewR = itR->first - x;
                    int knewR = itR->second - y;
                    seg.set_point(itR->first, A_mat(LnewR, knewR));
                }
                seg.set_point(x, A_mat(x - itL->first, y - aL));
                mp[x] = y;
                if (x > lastIdx) lastIdx = x;
                updChange();
            } else {
                if (it->second == y) {
                    updChange();
                ...
                auto itL = prev_it(it);
                auto itR = next(it);
                seg.set_point(x, A_mat(x - itL->first, y - itL->second));
                if (itR != mp.end()) {
                    seg.set_point(itR->first,
                                  A_mat(itR->first - x, itR->second - y));
                }
                it->second = y;
                updChange();
            }
