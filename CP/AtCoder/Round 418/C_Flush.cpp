...
void solve() {
    ...
    vl prefCnt(maxA + 1, 0), prefSum(maxA + 1, 0);
    rep(i, 1, maxA + 1) {
        prefCnt[i] = prefCnt[i - 1] + freq1[i];
        prefSum[i] = prefSum[i - 1] + 1LL * i * freq1[i];
    }
    ...
    while (Q--) {
        ...
        int t = b - 1;
        ll cntLE = (t >= 0 ? prefCnt[t] : 0);
        ll sumLE = (t >= 0 ? prefSum[t] : 0);
        ll sumMin = sumLE + 1LL * (N - cntLE) * t;
        print(sumMin + 1);
    }
}
