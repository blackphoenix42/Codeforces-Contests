 ...
 *    Created: 2025-08-16T17:43:53+05:30
 ...
 *    Problem Name: F - All Included
 *    Problem URL: https://atcoder.jp/contests/abc419/tasks/abc419_f
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    vs P;
    rep(i, 0, N) if (!drop[i]) P.pb(S[i]);

    int M = (int)P.size();
    if (M == 0) {
        ll ans = 1;
        rep(i, 0, L) ans = (ans * 26) % MOD;
        print(ans);
        return;
    }

    Aho ac;
    rep(i, 0, M) ac.add_string(P[i], i);
    ac.build();

    int states = (int)ac.t.size();
    int MASKS = 1 << M;
    int FULL = MASKS - 1;

    vi cur(states * MASKS, 0), nxt(states * MASKS, 0);
    cur[0 * MASKS + 0] = 1;

    for (int pos = 0; pos < L; ++pos) {
        fill(all(nxt), 0);
        rep(st, 0, states) {
            const auto &node = ac.t[st];
            rep(mask, 0, MASKS) {
                int val = cur[st * MASKS + mask];
                if (!val) continue;
                rep(c, 0, 26) {
                    int ns = node.nxt[c];
                    int nmask = mask | ac.t[ns].outmask;
                    int &cell = nxt[ns * MASKS + nmask];
                    int tmp = cell + val;
                    if (tmp >= MOD) tmp -= MOD;
                    cell = tmp;
                }
