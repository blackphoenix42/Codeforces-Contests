 ...
 *    Created: 2025-08-16T17:42:36+05:30
 ...
 *    Problem Name: E - Subarray Sum Divisibility
...
void solve() {
    ...
    const ll INF = (1LL << 60);
    vl dp(M, INF), ndp(M, INF);
    dp[0] = 0;
    for (int r = 1; r <= L - 1; ++r) {
        fill(all(ndp), INF);
        rep(s0, 0, M) {
            if (dp[s0] == INF) continue;
            rep(x, 0, M) {
                int t = s0 + x;
                if (t >= M) t -= M;
                ll val = dp[s0] + F[r][x];
                if (val < ndp[t]) ndp[t] = val;
            }
        }
        dp.swap(ndp);
    }
