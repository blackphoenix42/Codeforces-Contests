 ...
 *    Created: 2025-08-01 20:39:08
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - THIRD Programming Contest 2025 Summer（AtCoder Heuristic Contest 051）
 *    Problem Name: A - Probabilistic Waste Sorting
 *    Problem URL: https://atcoder.jp/contests/ahc051/tasks/ahc051_a
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    vector<pair<int, int>> sorter(M);
    for (int i = 0; i < M; i++) cin >> sorter[i].first >> sorter[i].second;
    ...
    for (int i = 0; i < M; i++) {
        long long dx = sorter[i].first - 0;
        long long dy = sorter[i].second - 5000;
    ...
    int imax = 0, imin = 0;
    for (int j = 1; j < N; j++) {
        if (p[0][j] > p[0][imax]) imax = j;
        if (p[0][j] < p[0][imin]) imin = j;
    }
