 ...
 *    Created: 2025-07-26 17:54:53
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Beginner Contest 416
 *    Problem Name: F - Paint Tree 2
 *    Problem URL: https://atcoder.jp/contests/abc416/tasks/abc416_f
 *    Time Limit: 3000 ms
...
void solve() {
    ...
    // rep(u, 1, N + 1) rep(t, 0, K + 1) dp0[u][t] = dp1[u][t] = NEG;

    vector<vector<array<ll, 3>>> dp0;
    dp0.assign(N + 1, vector<array<ll, 3>>(K + 2, {NEG, NEG, NEG}));
    ...
    for (int u : ord) {
        ...
        // ll f[6][3], nf[6][3];
        // FORN(k, 0, K) FORN(a, 0, 2) f[k][a] = NEG;
        // f[0][0] = 0;

        for (int v : adj[u]) {
            if (v == p[u]) continue;
            auto &dpv = dp0[v];
            // FORN(k, 0, K) FORN(a, 0, 2) nf[k][a] = NEG;
            vector<array<ll, 3>> dp1(K + 2, {NEG, NEG, NEG});
                ...
                FORN(a0, 0, 2) {
                    ...
                    for (int k1 = 0; k1 + k0 <= K; k1++) {
                        ...
                        if (c2 >= 0) {
                            if (a0 == 0) {
                                ll &ref = dp1[k0 + k1][1];
                                ref = max(ref, base + c2 + A[u]);
                            }
                            if (a0 == 1 && k0 + k1 + 1 <= K) {
                                ll &ref = dp1[k0 + k1 + 1][2];
                                ref = max(ref, base + c2);
                            }
                        }
                    }
                    ...
                    //         if (c1 <= NEG) continue;
                    //         nf[k0 + k1][a0 + 1] =
                    //             max(nf[k0 + k1][a0 + 1], base + c1);
                    //     }
                    // }
                }
            }
            dpu.swap(dp1);
            vector<array<ll, 3>>().swap(dpv);
        }
    }
    // FORN(t, 0, K) { dp0[u][t] = dp1[u][t] = NEG; }
    ...
    //         if (val <= NEG) continue;
    //         if (a == 0) {
    //             dp0[u][k0] = max(dp0[u][k0], val);
    //             dp1[u][k0] = max(dp1[u][k0], val + A[u]);
    //         } else if (a == 1) {
    //             if (k0 + 1 <= K)
    //                 dp0[u][k0 + 1] = max(dp0[u][k0 + 1], val + A[u]);
    //             dp1[u][k0] = max(dp1[u][k0], val + A[u]);
    //         } else {
    //             if (k0 + 1 <= K)
    //                 dp0[u][k0 + 1] = max(dp0[u][k0 + 1], val + A[u]);
