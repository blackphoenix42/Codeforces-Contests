 ...
 *    Created: 2025-07-26 18:15:03
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Beginner Contest 416
 *    Problem Name: G - Concat (1st)
 *    Problem URL: https://atcoder.jp/contests/abc416/tasks/abc416_g
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    string lex_min = S[0];
    for (int i = 1; i < N; i++) {
        if (S[i] < lex_min) lex_min = S[i];
    }

    auto better = [&](const string &a, const string &b) {
        int cmp = (a + b).compare(b + a);
    ...
    string s_star = S[0];
    for (int i = 1; i < N; i++) {
        if (better(S[i], s_star)) s_star = S[i];
    }
    ...
    if (K == 1) {
        ...
        string ans;
        ans.reserve((K - 1) * s_star.size() + lex_min.size());
    ...
    // string pref;
    // for (auto &s : S) {
    //     if (pref.empty() || s + minS < pref + minS) {
    //         pref = s;
    //     }
    // }
