...
void solve() {
    int N, M;
    cin >> N >> M;
    static ll road[501][501];
    rep(i, 1, N + 1) rep(j, 1, N + 1) road[i][j] = (i == j ? 0 : INF);
    rep(i, 0, M) {
        int a, b;
        ll c;
        cin >> a >> b >> c;
    ...
    rep(k, 1, N + 1) {
        rep(i, 1, N + 1) {
            ...
            rep(j, 1, N + 1) {
                ll nd = road[i][k] + road[k][j];
                if (nd < road[i][j]) road[i][j] = nd;
            }
        }
    }
    int K;
    ...
    while (Q--) {
        ...
        if (type == 1) {
            ...
            if (t < road[x][y]) {
                rep(i, 1, N + 1) {
                    ll dix = road[i][x], diy = road[i][y];
                    if (dix == INF && diy == INF) continue;
                    rep(j, 1, N + 1) {
                        if (dix != INF) {
                            ll alt = dix + t + road[y][j];
                            if (alt < road[i][j]) road[i][j] = alt;
                        }
                        if (diy != INF) {
                            ll alt = diy + t + road[x][j];
                            if (alt < road[i][j]) road[i][j] = alt;
                        }
                    }
                }
                road[x][y] = min(road[x][y], t);
                road[y][x] = min(road[y][x], t);
            }
        } else if (type == 2) {
            int x;
            cin >> x;
            ...
            static ll distFrom[501], distTo[501];
            rep(i, 1, N + 1) {
                distFrom[i] = INF;
                distTo[i] = INF;
            }
            for (int v : air) {
                rep(i, 1, N + 1) {
                    if (road[i][v] < distFrom[i]) distFrom[i] = road[i][v];
                    if (road[v][i] < distTo[i]) distTo[i] = road[v][i];
                }
            }
            ll ans = 0;
            rep(i, 1, N + 1) {
                rep(j, 1, N + 1) {
                    ll d = road[i][j];
                    if (!air.empty() && distFrom[i] != INF &&
                        distTo[j] != INF) {
                        ll viaA = distFrom[i] + T + distTo[j];
                        if (viaA < d) d = viaA;
                    }
                    if (d < INF) ans += d;
                }
