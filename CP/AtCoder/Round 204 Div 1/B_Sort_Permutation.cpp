 ...
 *    Created: 2025-08-17T17:48:06+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: Mind your words, they are important.
 *    Group: AtCoder - AtCoder Regular Contest 204 (Div. 1)
 *    Problem Name: B - Sort Permutation
 *    Problem URL: https://atcoder.jp/contests/arc204/tasks/arc204_b
 *    Time Limit: 2000 ms
...
static int cycle(const vi &cyc, int N) {
    const int m = sz(cyc);
    if (m <= 1) return 0;

    vi col(m);
    rep(t, 0, m) col[t] = (cyc[t] % N);

    vvi pos(N);
    rep(t, 0, m) pos[col[t]].pb(t);

    vector<uint16_t> dp(m * m, 0);
    auto at = [&](int l, int r) -> uint16_t & { return dp[l * m + r]; };

    rep(len, 2, m + 1) {
        for (int l = 0; l + len - 1 < m; ++l) {
            int r = l + len - 1;
            uint16_t best = at(l + 1, r);
            const auto &vec = pos[col[l]];
            auto it = ub(all(vec), l);
            for (; it != vec.end(); ++it) {
                int k = *it;
                if (k > r) break;
                uint16_t cand = at(l, k - 1) + at(k, r) + 1;
