 ...
 *    Created: 2025-08-17T17:54:55+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: Speak like a human.
 *    Group: AtCoder - AtCoder Regular Contest 204 (Div. 1)
 *    Problem Name: C - Maximize Sum of Mex
 *    Problem URL: https://atcoder.jp/contests/arc204/tasks/arc204_c
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    const ll C0 = sumE + sumOdd;

    int W = (int)sumE + 1;
    int WORDS = (W + 63) >> 6;
    vector<ull> dp(WORDS, 0ULL);
    dp[0] = 1ULL;
    ...
    auto sLeft = [&](int bits) {
        ...
        for (int i = WORDS - 1; i >= 0; --i) {
            ...
            if (src >= 0) {
                add |= dp[src] << rem;
                if (rem && src - 1 >= 0) add |= dp[src - 1] >> (64 - rem);
            }
            dp[i] |= add;
        }
    };
    ...
    vl ps_odd(1, 0);
    ps_odd.reserve(odd.size() + 1);
    for (int m : odd) ps_odd.pb(ps_odd.back() + m);

    auto getBit = [&](const vector<ull> &b, int idx) -> bool {
    ...
    auto maxC2 = [&](ll Z) -> ll {
        if (Z <= sumE) {
            if (Z == 0) return 0;
            return getBit(dp, (int)Z) ? Z : (Z - 1);
        } else if (Z <= C0) {
            ll need = Z - sumE;
            int k = int(lb(all(ps_odd), need) - ps_odd.begin());
            return Z - k;
        } else {
            ll d = Z - C0;
            int kC0 = int(lb(all(ps_odd), C0 - sumE) - ps_odd.begin());
            ll c2C0 = C0 - kC0;
            ll add = min(d, (ll)kOGe) - max(0LL, d - (ll)kOdd);
            ll val = c2C0 + add;
            if (val < 0) val = 0;
            if (val > (ll)N - Z) val = (ll)N - Z;
            return val;
        }
    };
    ...
    while (Q--) {
        ...
        ll Z = A0, O = A1;
        ll d = max(0LL, Z - C0);
        ll E00 = (d <= kOdd) ? d : (2 * d - kOdd);
        ll E0 = 2 * Z - E00;
        ll E01 = E0 - E00;
        ll c2 = maxC2(Z);
        ll Y = min(E01, O + min(O, c2));
        ll ans = E0 + Y;
        print(ans);
    }
}
