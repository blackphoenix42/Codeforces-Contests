 ...
 *    Created: 2025-08-17T17:58:05+05:30
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Quote: Accessible for all.
 *    Group: AtCoder - AtCoder Regular Contest 204 (Div. 1)
 *    Problem Name: D - Favorite Interval
 *    Problem URL: https://atcoder.jp/contests/arc204/tasks/arc204_d
 *    Time Limit: 2000 ms
...
void solve() {
    ...
    set<ll> S;
    for (ll x = K; x <= N - 1; ++x) S.insert(x);
    auto pMin = [&](ll m, ll a, ll b) -> ll {
        if (a > b || S.empty()) return -1;
        ll q = max<ll>(0, ceil_div((K - b), m));
        while (true) {
            ll lo = q * m + a;
            if (lo > N - 1) return -1;
            ll start = max(lo, K);
    ...
    for (ll t = 0; t < steps; ++t) {
        ll m = N - t;
        ll xL = -1, xR = -1;
        if (lRem > 0) xL = pMin(m, 0, lRem - 1);
        if (rRem > 0) xR = pMin(m, lRem + K, m - 1);
