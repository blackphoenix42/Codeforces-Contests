...
void solve() {
    ...
    rep(i, 1, N + 1) { cin >> P[i] >> A[i] >> B[i]; }
    const int MAXM = 1000;
    const int W = MAXM + 1;
    int rows = N + 2;
    vi g(rows * W);
    rep(m, 0, MAXM + 1) { g[(N + 1) * W + m] = m; }
    ...
    FORD(pos, N, 1) {
        rep(m, 0, MAXM + 1) {
            ...
            if (nm > MAXM) nm = MAXM;
            g[pos * W + m] = g[(pos + 1) * W + nm];
        }
    }
    vl preB(N + 1);
    preB[0] = 0;
    rep(i, 1, N + 1) { preB[i] = preB[i - 1] + B[i]; }
    int Q;
    cin >> Q;
    while (Q--) {
        ll X;
        ...
        if (X <= MAXM) {
            ...
            ll need = X - MAXM;
            auto it = lb(preB.begin() + 1, preB.begin() + N + 1, need);
            if (it == preB.begin() + N + 1) {
                ans = X - preB[N];
            } else {
                int k = int(it - preB.begin());
                ll moodPre = X - preB[k];
                if (moodPre < 0) moodPre = 0;
                if (moodPre > MAXM) moodPre = MAXM;
                ans = g[(k + 1) * W + (int)moodPre];
            }
        }
        print(ans);
    }
