...
struct mine {
    ...
    mine operator*(const mine &o) const { return mine(v * o.v % MOD); }
...
void solve() {
    ...
    vm inv(N + 2);
    rep(i, 1, N + 1) inv[i] = mine(mod_pow(i));

    map<int, pair<int, mine>> blocks;
    rep(i, 1, N + 1) blocks[i] = {i, A[i]};

    auto split = [&](int pos) {
        if (pos > N) return;
        auto it = blocks.ub(pos);
        if (it == blocks.begin()) return;
        --it;
        int l = it->first;
        int r = it->second.first;
        mine val = it->second.second;
        if (l == pos) return;
        if (r < pos) return;
    ...
    rep(i, 1, M + 1) {
        ...
        mine total = mine(0);
        vi toerase;
        while (it != blocks.end() && it->first <= r) {
            int bl = it->first;
            int br = it->second.first;
            mine val = it->second.second;
            int len = br - bl + 1;
            total += val * mine(len);
            toerase.pb(bl);
        ...
        for (int key : toerase) blocks.erase(key);
        int len = r - l + 1;
        mine newval = total * inv[len];
        blocks[l] = {r, newval};
    }

    vm ans(N + 1);
    for (auto &[l, pr] : blocks) {
        int r = pr.first;
        mine v = pr.second;
        rep(i, l, r + 1) { ans[i] = v; }
    }
