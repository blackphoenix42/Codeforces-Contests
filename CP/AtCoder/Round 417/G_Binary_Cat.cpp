 ...
 *    Created: 2025-08-02 17:44:09
 *    Profile: https://codeforces.com/profile/BinaryPhoenix10
 *    Group: AtCoder - AtCoder Beginner Contest 417
 *    Problem Name: G - Binary Cat
 *    Problem URL: https://atcoder.jp/contests/abc417/tasks/abc417_g
 *    Time Limit: 6000 ms
...
void solve() {
    ...
    int nodes = Q + 2;
    vi L(nodes), R(nodes);
    vector<ull> len(nodes);
    static int up[500005 + 5][20];
    static ull delta[500005 + 5][20];
    static ull low[500005 + 5][20];
    static ull high[500005 + 5][20];
    ...
    rep(i, 1, Q + 1) {
        ...
        bool isLeft = (l_len >= r_len);
        int big = isLeft ? L[k] : R[k];
        up[k][0] = big;
        if (isLeft) {
            low[k][0] = 1;
            high[k][0] = l_len;
        ...
        rep(j, 1, 20) {
            ...
            if (mid == 0) {
                ...
                up[k][j] = up[mid][j - 1];
                ull d1 = delta[k][j - 1];
                ull d2 = delta[mid][j - 1];
                ull dsum = d1 + d2;
                if (dsum >= INF) dsum = INF;
                delta[k][j] = dsum;
                ull lo1 = low[k][j - 1];
                ull hi1 = high[k][j - 1];
                ull lo2 = low[mid][j - 1];
                ull hi2 = high[mid][j - 1];
                ull mp_lo2, mp_hi2;
                if (lo2 > hi2) {
                    mp_lo2 = 1;
                    mp_hi2 = 0;
                } else {
                    mp_lo2 = lo2 + delta[k][j - 1];
                    if (mp_lo2 < lo2) mp_lo2 = INF;
                    mp_hi2 = hi2 + delta[k][j - 1];
                    if (mp_hi2 < hi2) mp_hi2 = INF;
                }
                ull new_lo = max(lo1, mp_lo2);
                ull new_hi = min(hi1, mp_hi2);
                if (new_lo > new_hi) {
                    low[k][j] = 1;
                    high[k][j] = 0;
                } else {
        ...
        while (true) {
            ...
            FORD(j, 20 - 1, 0) {
                if (low[node][j] <= cur_x && cur_x <= high[node][j]) {
                    cur_x -= delta[node][j];
                    node = up[node][j];
                }
            }
